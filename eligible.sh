...
# (aka E23) on Ubuntu Bionic Beaver; or helps you perform a clean uninstall of E23.
...
# 4. Then type ./eligible.sh

# Subsequent runs.
# Open Terminal and simply type eligible.sh
...
# and delete any leftover files) before running ELIGIBLE.SH.
...
# ELIGIBLE.SH is written by similar@orange.fr, feel free to use this script as you see fit.
...
libudisks2-dev libunibreak-dev libunwind-dev libuv1-dev libvlc-dev \
libwebp-dev libxcb-keysyms1-dev libxcursor-dev libxine2-dev libxinerama-dev \
libxkbcommon-x11-dev libxkbfile-dev libxrandr-dev libxss-dev libxtst-dev \
linux-tools-common lolcat manpages-dev python3-setuptools python3-wheel \
...
do_tests() {
  ...
  if systemd-detect-virt -q --container; then
    printf "\n$BDR%s %s\n" "ELIGIBLE.SH IS NOT INTENDED FOR USE INSIDE CONTAINERS."
    printf "$BDR%s $OFF%s\n\n" "SCRIPT ABORTED."
    beep_exit
    exit 1
  fi
